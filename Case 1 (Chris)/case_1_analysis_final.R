...
# Analysis for Case 1 Tomato data.
...
# Scatter plots
ggplot(tomato_data) +
  geom_point(mapping = aes(x = diam_stem_mm, 
                           y = leafarea_cm2, 
                           colour = type.))

ggplot(tomato_data) +
  geom_point(mapping = aes(x = leafarea_cm2,
                           y = firmness_log_mm, 
                           colour = type.))

ggplot(tomato_data) +
  geom_point(mapping = aes(x = firmness_log_mm, 
                           y = leafarea_cm2, 
                           colour = type.))
...
hist(tomato_data$diam_stem_mm, 
     xlab = "Stem diameter in mm",
     main = "", 
     breaks = 5)

hist(tomato_data$leafarea_cm2,
     xlab = "Leaf area in cm2", 
     main = "",
     breaks = 5)

hist(tomato_data$firmness_log_mm, 
     xlab = "Log(firmness in mm)",
     main = "", 
     breaks = 5)
...
stem_lm <- nlme::lmList(diam_stem_mm ~ firmness_log_mm * leafarea_cm2 | type., 
                        tomato_data)

leafarea_lm <- nlme::lmList(leafarea_cm2 ~ firmness_log_mm * diam_stem_mm | type., 
                            tomato_data)
